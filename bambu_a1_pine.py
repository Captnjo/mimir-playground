#!/usr/bin/env python3
"""
Bambu Lab A1 - DEFINITIVE MINIMAL WORKING SCRIPT (Pine A64 Version)

This version downloads the cert automatically or uses a local path.
"""
...
# Try to find or download the certificate
CERT_FILE = "bambu.cert"
CERT_URL = "https://raw.githubusercontent.com/Captnjo/mimir-playground/main/bambu_certs/bambu.cert"
# ======================================================

# Download cert if not present
if not os.path.exists(CERT_FILE):
    print(f"[SETUP] Downloading certificate from {CERT_URL}...")
    try:
        urllib.request.urlretrieve(CERT_URL, CERT_FILE)
        print(f"[SETUP] Certificate saved to {CERT_FILE}")
    except Exception as e:
        print(f"[ERROR] Failed to download certificate: {e}")
        print("[ERROR] Please manually place bambu.cert in the current directory")
        sys.exit(1)
